...
void generateLinePoints(glm::vec2 start_point, glm::vec2 direction, GLint num_segments) {
	GLfloat		x, y;
	GLfloat		t			= 0.0f;
	GLfloat		step		= 1.0f / (GLfloat)num_segments;

	/** num_segments + 1 pontunk lesz. */
	/** There will be num_segments + 1 points. */
	for (int i = 0; i <= num_segments; i++) {
		/** Vegyük a start_point-ot és haladjunk step méretû lépésekkel. */
		/** Take start_point and increment it step by step. */
		x = start_point.x + t * direction.x;
		y = start_point.y + t * direction.y;

		/** Tároljuk el az új (x, y, 0.0) pontot. */
		/** Let's store (x, y, 0.0) new point. */
		linePoints.push_back(glm::vec3(x, y, 0.0f));

		/** Léptessük t-t step-el. */
		/** Increase t by step. */
		t += step;
	}
}
...
void init(GLFWwindow* window) {
	...
	/** Generáljunk 5 pontot egy szakasz 4 részéhez. */
